...
이 DAG는 ``--all-codes``로 daily_bars 전 종목(~2,600개)을, ``--from-year 2016``부터(주1)
전체 이력으로 한 번에 ``earnings`` 테이블에 upsert하는 **백필 전용** 파이프라인이다.
일상적인 최신 분기 반영(최신 유지)은 ``daily_earnings``가 담당한다 — 두 목적(최신
유지 vs. 과거 이력 채우기)이 달라 별도 DAG로 분리했다.
