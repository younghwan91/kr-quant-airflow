...
한국 주식(코스피·코스닥)의 **시세·수급·실적·컨센서스** 데이터를 매일 자동 수집해
TimescaleDB에 적재하는 Airflow 오케스트레이션 레포. 수집 로직(`collectors/`)과
스케줄링(`dags/`)을 자체 보유한다.
...
수집 로직을 분석 레포에서 떼어 이 레포에 둔 이유:

1. **사고 방지** — 분석 세션에서 실수로 수집기를 직접 실행해 DB 정합성이 깨지는
   일을 막는다. 분석(kr-quant)과 수집(이 레포)은 완전히 분리된 프로세스·레포다.
2. **오픈소스 공유** — 수집 로직을 공개한다. `collectors/`는 `kr_quant` 패키지에
   대한 런타임 의존이 전혀 없다(자체 `collectors/storage.py`·`collectors/config.py` 보유).

> **예외** — kr-quant는 여전히 `/opt/kr-quant`로 (읽기 전용) 마운트된다.
> `daily_minervini_scan`(scanner_final.py 전략 로직)과
> `weekly_price_adjust`(kr_quant.price_adjust 백조정 로직) 2개 DAG가 kr-quant의
> analysis 코드를 in-place로 실행하기 때문이다(PYTHONPATH/sys.path 기반, 패키지 설치 아님).
...
머신 가동은 cron이 관리한다 — 매일 10:00 스택을 올리고, `scripts/wait_and_stop.sh`가
그날 예정된 DAG가 전부 끝나면 조기 종료한다(22:00 안전장치 포함). 모든 DAG 스케줄은
이 가동 창 안에 든다.
...
git clone https://github.com/younghwan91/kr-quant.git ../kr-quant   # sibling — 2개 DAG만 사용
cd kr-quant-airflow
...
데이터는 **매일 증분 + 주간 깊이 재수집**의 2단 구조다. 평일 DAG가 최신 데이터를
증분으로 채우고, 주간 DAG가 히스토리 깊이를 유지해 신규 상장 종목·새 지수·DB 리셋
후에도 과거가 비지 않게 한다(모든 수집기가 `(code, date)` upsert라 idempotent).
...
> **신뢰성** — 모든 DAG 태스크에 재시도가 걸려 있다: 외부 API·수집 DAG는
> `retries=1, retry_delay=10분`, 전체 이력 백필은 `retries=2, 30분`. 일시적 네트워크
> 오류로 그날 데이터가 조용히 빠지는 것을 방지한다.
>
...
전체 정의는 [`sql/init_timescale.sql`](sql/init_timescale.sql). 시계열 테이블은
TimescaleDB hypertable(PK `(code, date)`), 그 외는 일반 테이블이다.
...
**`dags/_common.py`** — DAG마다 복붙하던 DSN/자격증명 헬퍼를 한곳에 모았다.
`run_collector()`는 수집기 stdout을 태스크 로그로 실시간 스트리밍하고, 로그에 남는
DSN 비밀번호를 마스킹한다(`collectors/config.py`의 `mask_dsn`을 재사용 — 단일 소스).

## TimescaleDB 설계 노트

- **청크 크기** — 모든 hypertable은 `chunk_time_interval = 1년`. 기본 7일 청크는 이
  데이터 볼륨(~2,600종목 × 250거래일/년 ≈ 65만 행/년)엔 과하게 잘게 쪼개
  청크 메타데이터 오버헤드 + 여러 해 스캔이 느려진다.
- **압축** — 시세·수급·컨센서스 hypertable은 7일 지난 청크를 컬럼형으로 자동 압축한다
  (`compress_segmentby = 'code'`). 최근 데이터는 행 기반으로 두어 잦은 upsert를 빠르게.
- **`daily_bars_adjusted`는 압축 제외** — `weekly_price_adjust`가 매주 전체를 upsert로
  재작성하므로, 압축을 걸면 매주 오래된 청크를 압축해제→재압축하는 순환만 반복된다.
- **DB 쓰기** — 수집기는 `psycopg2.extras.execute_values`로 배치 upsert하고, 긴 전수
  수집은 청크 단위(100종목) 중간 커밋으로 크래시 시 손실을 제한한다.
...
분석·백테스트 코드는 [kr-quant](https://github.com/younghwan91/kr-quant)에 있으며,
이 DB를 읽기 전용으로 사용한다.
...
`airflow-init` 컨테이너에서만 한 번 읽어 `airflow variables set`으로 Airflow 메타DB에
**Fernet 암호화** 저장하고, DAG 태스크가 실행 시점에 `Variable.get()`으로 꺼내 수집기
서브프로세스 환경에만 주입한다(`dags/_common.py`의 `kiwoom_env()`/`dart_env()`).

TimescaleDB 접속 정보(`TIMESCALE_*`)는 LAN 내부용이라 평문 컨테이너 env로 뒀다 —
필요하면 Airflow Connection으로 옮길 수 있지만 지금 스코프에선 과하다.

`.env`는 절대 커밋하지 않는다(`.gitignore` 포함).
